...
describe('Paying Customers Helpers', () => {
  describe('#getPayingCustomersDataByMonth()', () => {
    ...
    it('should throw if one of the arguments is not a Map', () => {
      expect(
        getPayingCustomersDataByMonth.bind(null, 'data', Map())
      ).toThrow('Wrong type of argument(s)')
      expect(
        getPayingCustomersDataByMonth.bind(null, Map(), 'range')
      ).toThrow('Wrong type of argument(s)')
      expect(
  ...
  describe('Get Paying Customers Data By Month Helpers', () => {
    ...
    describe('#getMonthDateStringsInRange()', () => {
      ...
      it('should have number of entries according to number of months in range', () => {
        const from = +moment('01/01/16', 'DD/MM/YY').format('x')
        const to = +moment('01/03/16', 'DD/MM/YY').format('x')
    ...
    describe('#firstMonthSum()', () => {
      ...
      it('should throw is missing or incorrect arguments', () => {
        ...
        expect(firstMonthSum.bind(null, 'data', moment(from).format('YY/MM/DD'))).toThrow('missing or incorrect arguments')
        expect(firstMonthSum.bind(null, data, 'from')).toThrow('missing or incorrect arguments')
        expect(firstMonthSum.bind(null, data, moment(from).format('YY/MM/DD'))).toNotThrow()
      ...
      it('should return sum of numbers in range', () => {
        ...
        while (curr.isSameOrBefore(endOfMonth)) {
          ...
          data = data.setIn(path, number)
          values.push(number)
          curr = curr.clone().add(1, 'day')
        }

        expect(
          firstMonthSum(data, endOfMonthMinus2Days.format('x'))
        ).toBe(sum(values))
      })
    })
    ...
    describe('#lastMonthSum()', () => {
      ...
      it('should throw is missing or incorrect arguments', () => {
        ...
        expect(lastMonthSum.bind(null, 'data', moment(to).format('YY/MM/DD'))).toThrow('missing or incorrect arguments')
      ...
      it('should return sum of numbers in range', () => {
        ...
        expect(
          lastMonthSum(data, threeDaysLater.clone().format('x'))
        ).toBe(sum(values))
      })
    })
    ...
    describe('#singleMonthSum()', () => {
      ...
      beforeEach(() => {
        from = moment().startOf('month').add(3, 'days')
        to = from.clone().add(2, 'days')
        let curr = from.clone()
    ...
    describe('#fullMonthsTotals()', () => {
      ...
      beforeEach(() => {
        from = moment().startOf('year').add(3, 'days')
        to = moment()
        let curr = from.clone()

        // (from.format('YY/MM/DD'))
        // console.log(to.format('YY/MM/DD'))
      ...
      it('should return List', () => {
        expect(
          fullMonthsTotals(data, List.of('16/05/01', '16/06/01', '16/07/01'))
        ).toBeA(List)
      })
